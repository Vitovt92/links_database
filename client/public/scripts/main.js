 var slideIndex = []; // чтобы у каждого документа был свой   slideIndex 
...
function makeTable() {
   ...
   "<div class='panel'>"+
       "<div>"+
       "<ul class='menuDox'>"+ addColumnToTable('akt', '1' ) + addColumnToTable('tu', '2') + addColumnToTable('dog', '3') + addColumnToTable('map', '4') + addProgectToTable() + addMapToTable()        
          + "</ul>"+
       "</div>"+
"<div class='fotoOfdox' id='fotoOfdox"+String(l)+"'></div>"+
 ...
 // строит всплывающее меню в таблице   
function addColumnToTable(nameOfColumn,namberOfColumn) {
    
    var NameOfColomnInUkreinian = "";
  switch(nameOfColumn){
      case 'akt': 
          NameOfColomnInUkreinian = "Aкты";
          break;
      case 'tu':
          NameOfColomnInUkreinian = "ТУ";
          break;
      case 'dog':
          NameOfColomnInUkreinian = "Договора";
          break;
      case 'map':
          NameOfColomnInUkreinian = "Картограмма";
          break;
  }   
    
            for (var propInCableObj in cable[l].urid){

                var generetedColumn ="";
                
    if ( (propInCableObj ==  nameOfColumn) && !(cable[l].urid[nameOfColumn].url[0] === undefined)  ){
         generetedColumn = "<li class='menuDox'><a class='tablinks"+String(l)+"' onclick='showSlides(slideIndex["+String(l)+"]," + namberOfColumn + ","+String(l)+"), difrentColors(event,"+String(l)+")'>" + NameOfColomnInUkreinian + "</a></li>"
        break;
        };
                
         generetedColumn = "<li class='menuDox'><a class='inaktiveColumn'>" + NameOfColomnInUkreinian + "</a></li>";        
        continue;
                
      }  
    return generetedColumn;
 }
    
function addProgectToTable() {
   
    if(cable[l].urid.ex[1].url[0]){
        return "<li class='menuDox'><a class='tablinks"+String(l)+"' onclick='showSlides(slideIndex["+String(l)+"],5,"+String(l)+"), difrentColors(event,"+String(l)+")' >Проект</a></li>";
    }
    return "";
    
}
    
function addMapToTable(){
    if(cable[l].fiz.path[0]){
        return "<li class='menuDox'><a class='tablinks"+String(l)+"' onclick='difrentColors(event,"+String(l)+"), initMap("+String(l)+"), myMap("+String(l)+")' >Карта</a></li>";
    }
    return "";
}
...
function showSlides(n,type,nambOfDox) {
  ...
  var infoDoc
   // Разные значения в зависимости от выбора
    ...
    else if(type==5){ //если проект
        infoArr=cable[nambOfDox].urid.ex[1].url;
		infoDoc=cable[nambOfDox].urid.ex[1].doc;
    }
	// Цикл перебирает и подставляет все фото
...
cablePath.setMap(map);  // Метод Инициализирует путь кабеля
